...
namespace FSWApi
{
	...
	internal class FSWContext : DbContext
	{
		private string _dataBaseFileName;
		//private DbSet<FileMetaInfo> _fileMetaInfos;

		public FSWContext(string fileName)
		{
	...
	public class DataBase
	{
		public DataBase(string fileName)
		{
			...
			DbContext dbContext = new FSWContext(fileName);
			dbContext.Database.EnsureCreated();
		}

		internal void AddFileMetaInfo(FileMetaInfo fileMetaInfo, string dataBaseFileName)
		{
			using (var dbContext = new FSWContext(dataBaseFileName))
			{
